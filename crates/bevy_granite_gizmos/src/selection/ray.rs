...
use bevy::prelude::{Entity, Name, Query, Res, ResMut, Resource, Vec3};
...
pub fn raycast_at_cursor(
    ...
    if let Some(cursor_ray) = ***cursor_ray {
        ...
        let gizmo_filter =
            |entity: Entity| query.get(entity).is_ok_and(|(_, gizmo, _, _)| gizmo.is_some());

        let icon_filter =
            |entity: Entity| query.get(entity).is_ok_and(|(_, gizmo, icon, _)| gizmo.is_none() && icon.is_some());
